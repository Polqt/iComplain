...
function createWebSocketStore() {
	...
	function handleMessage(event: MessageEvent) {
		try {
			...
			if (data.type === "comment_created") {
				...
				if (data.comment) {
					commentsStore.updateCommentInStore(data.comment.id, {
	...
	function scheduleReconnect() {
		...
		reconnectAttempts++;
		console.log(
			`WebSocket disconnected. Attempting to reconnect in ${delay / 1000} seconds... (Attempt ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})`,
		);
