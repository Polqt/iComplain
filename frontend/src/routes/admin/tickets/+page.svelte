<script lang="ts">
  ...
  const categories = ["Facilities", "IT Support", "Safety", "Cleanliness", "Equipment"];
  const locations = [
    "Building A - 1F",
    "Building A - 3F",
    "Building B - 2F",
  ...
  $: selectedTicket = tickets.find((ticket) => ticket.id === selectedId) ?? null;
</script>
...
<AdminLayout>
  <div class="flex flex-col h-[calc(100vh-8rem)] gap-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 min-h-0">
      <section class="card bg-base-100 dark:bg-base-100 shadow-sm border border-base-content/5 rounded-lg overflow-hidden">
        <div class="card-body p-0 h-full">
          <div class="p-4 border-b border-base-content/5">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                ...
                <span class="badge badge-sm badge-ghost">{filteredTickets.length}</span>
              </div>
          ...
          <div class="flex flex-col overflow-y-auto h-full">
            {#each filteredTickets as ticket}
              <button
                type="button"
                class="w-full text-left px-4 py-3 border-b border-base-content/5 hover:bg-base-200 transition-colors {ticket.id === selectedId ? 'bg-base-200' : ''}"
                onclick={() => (selectedId = ticket.id)}
              >
                <div class="flex items-center justify-between mb-2">
                  <span class="text-xs font-semibold">{ticket.id}</span>
                  <span class="text-xs text-base-content/50">{ticket.updatedAt}</span>
                </div>
                <div class="flex items-center justify-between gap-2">
                  ...
                  {#if ticket.unread > 0}
                    <span class="badge badge-error badge-sm">{ticket.unread}</span>
                  {/if}
                </div>
              </button>
            {/each}
      ...
      <section class="card bg-base-100 dark:bg-base-100 shadow-sm border border-base-content/5 rounded-lg lg:col-span-2 overflow-hidden">
        <div class="card-body p-4 h-full">
          {#if selectedTicket}
            <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
              ...
              <div class="flex items-center gap-2">
                <span class="badge badge-sm {statusConfig[selectedTicket.status].color}">
                  {statusConfig[selectedTicket.status].label}
                </span>
                <span class="badge badge-sm {priorityConfig[selectedTicket.priority].color}">
          ...
          {:else}
            <div class="flex flex-col items-center justify-center h-full text-base-content/60">
